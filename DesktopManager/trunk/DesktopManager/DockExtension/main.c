...
void INJECT_ENTRY( ptrdiff_t offset, void *paramBlock, size_t paramSize ) {
    ...
    err = AEInstallEventHandler(
								...
								kWindowControllerPing,
								NewAEEventHandlerUPP((&pingHandler) + offset),
								0, FALSE
								);
    if(err) { syslog(LOG_ERR, "Error registering ping handler: %i", err); }

    err = AEInstallEventHandler(
        kWindowControllerClass,
        kWindowControllerHideWindow,
        NewAEEventHandlerUPP((&hideWindowHandler) + offset),
        0, FALSE
    );
    if(err) { syslog(LOG_ERR, "Error registering hide handler: %i", err); }

    err = AEInstallEventHandler(
        kWindowControllerClass,
        kWindowControllerShowWindow,
        NewAEEventHandlerUPP((&showWindowHandler) + offset),
        0, FALSE
    );
    if(err) { syslog(LOG_ERR, "Error registering show handler: %i", err); }
    ...
    err = AEInstallEventHandler(
								kWindowControllerClass,
								kWindowControllerCreateAnimation,
								NewAEEventHandlerUPP((&createAnimationHandler) + offset),
								0, FALSE
								);
    if(err) { syslog(LOG_ERR, "Error registering move handler: %i", err); }    
    ...
    err = AEInstallEventHandler(
								kWindowControllerClass,
								kWindowControllerFreeAnimation,
								NewAEEventHandlerUPP((&freeAnimationHandler) + offset),
								0, FALSE
								);
    if(err) { syslog(LOG_ERR, "Error registering move handler: %i", err); }    
    ...
    err = AEInstallEventHandler(
								kWindowControllerClass,
								kWindowControllerRunAnimation,
								NewAEEventHandlerUPP((&performAnimationsHandler) + offset),
								0, FALSE
								);
    if(err) { syslog(LOG_ERR, "Error registering move handler: %i", err); }    
